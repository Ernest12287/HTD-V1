...
function getClientIp(req) {
    // Prioritized IP detection methods
    const ipHeaders = [
        'x-forwarded-for',
        'x-real-ip',
        'cf-connecting-ip',
    ...
    for (const header of ipHeaders) {
        ...
        if (ip) {
            // Split and take the first IP if multiple are present
            ...
            // Validate IP format and exclude private/local IPs
    ...
    // Fallback to request IP methods
    let fallbackIp = req.ip || 
                    req.connection.remoteAddress || 
                    req.socket.remoteAddress;

    // Remove IPv6 to IPv4 mapping prefixes
...
// Updated IP-based account tracking
async function checkAndTrackIpSignup(ip, connection) {
    ...
    try {
        ...
        // Check existing IP tracking record
        ...
        // If recent and exceeded max accounts, block signup
        if (record.is_recent && record.account_count >= MAX_ACCOUNTS_PER_IP) {
            return false; // Reject the signup attempt
        }

        // If not recent or under the limit, allow signup and update tracking
        await connection.query(
...
router.post('/signup', async (req, res) => {
    ...
    try {
        ...
        // Get client IP with improved method
        ...
        // Store verification details (including IP)
        verificationCodes.set(email, {
            ...
            clientIp // Store the IP for later verification
        });
