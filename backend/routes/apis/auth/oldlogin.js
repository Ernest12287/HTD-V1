...
// Use Map for verification codes
...
router.post('/login', async (req, res) => {
    ...
    try {
        ...
        // Check if account is banned - Return JSON response instead of rendering
        // if (user.is_banned) {
        //     req.session.user = {
        //         is_banned: 1, 
        //     }
        ...
        //         message: 'Your account has been banned for violating our terms of service.'
        //     });
        // }
        ...
        if (knownDevices.length > 0) {
            // Known device - allow direct login
            await pool.query(
        ...
        // New device - create verification process
        ...
        // Create unverified device record
        await pool.query(
...
router.post('/verify-email-login', async (req, res) => {
    ...
    try {
        ...
        return res.status(200).json({
            ...
            user: req.session.user // Optional: Send session user data in response
        });
...
// Cleanup function for verification codes
