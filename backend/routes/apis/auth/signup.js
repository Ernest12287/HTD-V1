...
function getClientIp(req) {
    ...
    for (const header of ipHeaders) {
        ...
        if (ip) {
            ...
            if (/^(\d{1,3}\.){3}\d{1,3}$/.test(possibleIp) && !/^(127\.|10\.|192\.168\.|172\.(1[6-9]|2[0-9]|3[0-1])\.)/.test(possibleIp)) {
                return possibleIp;
            }
        }
    }
...
router.post('/verify-signup', async (req, res) => {
    ...
    try {
        ...
        const [ipRecords] = await connection.query(
            `SELECT account_count FROM ip_account_tracking 
             WHERE ip_address = ? AND DATEDIFF(NOW(), last_signup) <= 30
             FOR UPDATE`,
            [verification.clientIp]
        );

        const shouldBanAccount = ipRecords.length > 0 && ipRecords[0].account_count >= 1;
        ...
        req.session.user = {
            ...
            isVerified: true,
            is_banned: shouldBanAccount,
            is_admin: 0
        };

        return res.json({
            success: true,
            banned: shouldBanAccount,
            message: shouldBanAccount
                ? 'We are having problem with your account, please contact support!'
                : 'Registration successful!',
            user: !shouldBanAccount ? {
                id: userId,
                email: normalizedEmail,
